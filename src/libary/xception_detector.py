...
if __name__ == "__main__":
    ...
    model = XceptionDetector(num_classes=2).to(device)
    
    # Tạo batch giả lập gồm 4 ảnh RGB size 256x256 và nhãn nhị phân
    fake_images = torch.randn(4, 3, 256, 256).to(device)
    fake_labels = torch.randint(0, 1, (4,)).to(device)

    data_dict = {'image': fake_images, 'label': fake_labels}
