...
def process_video():
    ...
    try:
        ...
        url = data['url']
        file_path, _ = download_video(url)  
        process_video = RealTimeVisionSystem(model, tracker, face_detector, face_predictor, file_path, threshold_fake, None, 30, "fixed_stride")
        video_file, prob_file, imgs_folder, result = process_video.run()
        os.remove(file_path)

        # Trả về URL cho video output
        video_name = os.path.basename(video_file)
        return jsonify({'output_path': f"https://firefly-genuine-adequately.ngrok-free.app/output?v={video_name.split('.')[0]}",
                        'result': f"{result}"})
...
def render_video():
    try:
        ...
        return render_template('output.html', video_path=video_path, props_data=probs_data, imgs_path=imgs_path, threshold_fake=threshold_fake)
    except Exception as e:
